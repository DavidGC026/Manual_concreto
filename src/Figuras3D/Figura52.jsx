...
const ThreeScene = () => {
    ...
    useEffect(() => {
        ...
        const blueRodPoints2 = [
            new THREE.Vector3(4.5, -4.25, -0.75),
            new THREE.Vector3(4.5, -3.75, -0.75),
            new THREE.Vector3(4.5, -3.25, -0.75),
        ...
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 0.8 - 4.15, 6, 20.75);
        ...
        const purpleRodPoints = [
            ...
            new THREE.Vector3(4.5, -1.75, -0.25),
        ...
        // Crear varillas moradas
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 2.15 - 3.45, 2, -3.85);
        ...
        // Crear varillas amarillas
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 0.8 - 4.15, 2, 20.75);
        ...
        // Crear varillas amarillas
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 0.8 - 4.15, 2, -3.85);
        ...
        for (let i = 0; i < 4; i++) {
            const offset = new THREE.Vector3(0, 3.4, -1.6 - i * 2);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }

        for (let i = 0; i < 4; i++) {
            const offset = new THREE.Vector3(0, 3.4, -0.8 - i * 2);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }

        for (let i = 0; i < 4; i++) {
            const offset = new THREE.Vector3(0, 3.4, 18.2 - i * 2);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }

        for (let i = 0; i < 4; i++) {
            const offset = new THREE.Vector3(0, 3.4, 17.4 - i * 2);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }
        ...
        for (let i = 0; i < 4; i++) {
            const offset = new THREE.Vector3(0, 3.4, 11.45 - i * 3.5);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }

        for (let i = 0; i < 3; i++) {
            const offset = new THREE.Vector3(0, 3.4, 9.55 - i * 3.5);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }
        ...
        for (let i = 0; i < 3; i++) {
            const offset = new THREE.Vector3(0, 3.4, -13.6 - i * 2);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }

        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(0, 3.4, -14.8 - i * 2);
            const points = redRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xff0000, 0.1));
        }
