...
const ThreeScene = () => {
    ...
    useEffect(() => {
        ...
        const blueRodPoints = [
            new THREE.Vector3(4.5, -4.75, -0.5),
            new THREE.Vector3(4.5, -3.75, -0.5),
        ...
        const purpleRodPoints = [
            new THREE.Vector3(4.5, -0.5, 0.25),
            new THREE.Vector3(4.5, -0.75, 0.25),
            new THREE.Vector3(4.5, -1.5, 0.25),
            new THREE.Vector3(4.5, -2, 0.25),
            new THREE.Vector3(4.5, -2.25, 0.25),
            new THREE.Vector3(4.5, -2.75, 0.25),
            new THREE.Vector3(4.5, -2.75, -0.5),
            new THREE.Vector3(4.5, -2.75, -1),
            new THREE.Vector3(4.5, -2.75, -25.5),
            new THREE.Vector3(4.5, -2.75, -26),
            new THREE.Vector3(4.5, -2.5, -28),
            new THREE.Vector3(4.5, -2.5, -28.5),
            new THREE.Vector3(4.5, -2.5, -34.5),
        ];

        // Crear varillas moradas
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 1.5 - 3.8, 2, 20.75);
        ...
        // Crear varillas moradas
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 1.5 - 3.8, 2, -3.85);
        ...
        for (let i = 0; i < 1; i++) {
            const offset = new THREE.Vector3(-i * 1.1 - 4.5, 2, 20.75);
        ...
        const greenRodPoints = [
            ...
            new THREE.Vector3(-1.2, 4.4, 0),
            new THREE.Vector3(-1.1, 4.4, 0),
            new THREE.Vector3(-1.05, 4.4, 0),

            new THREE.Vector3(-0.95, 4.4, 0),
            new THREE.Vector3(-0.95, 2, 0),
        ...
        for (let i = 0; i < 6; i++) {
            const offset = new THREE.Vector3(0, -0.95, 18 - i);
            const points = greenRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
        // Crear varillas verdes
        for (let i = 0; i < 3; i++) {
            const offset = new THREE.Vector3(0, -0.95, 11.25 - i * 2);
            const points = greenRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
        // Crear varillas verdes
        for (let i = 0; i < 4; i++) {
            const offset = new THREE.Vector3(0, -0.95, 1.5 - i * 2);
            const points = greenRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
        ...
        // Crear varillas verdes
        for (let i = 0; i < 3; i++) {
            const offset = new THREE.Vector3(0, -0.95, -6 - i);
            const points = greenRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
        ...
        // Crear varillas verdes
        for (let i = 0; i < 4; i++) {
            const offset = new THREE.Vector3(0, -0.95, -14 - i);
            const points = greenRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
