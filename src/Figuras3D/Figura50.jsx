...
const ThreeScene = () => {
    ...
    useEffect(() => {
        ...
        // Crear varillas azules
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 2.1 - 3.45, 6, -3.85);
        ...
        const purpleRodPoints = [
            new THREE.Vector3(4.5, -2.75, 0.25),
            new THREE.Vector3(4.5, -2.75, -0.5),
            new THREE.Vector3(4.5, -2.75, -1),
            new THREE.Vector3(4.5, -2.75, -31),
        ];

        // Crear varillas moradas
        for (let i = 0; i < 3; i++) {
            const offset = new THREE.Vector3(-i * 1.1 - 3.45, 2, 20.75);
            const points = purpleRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0xca5cdd, 0.1));
        }

        // Varillas moradas optimizadas con TubeGeometry
        const purpleRodPoints2 = [
            new THREE.Vector3(4.5, -2.75, -8),
            new THREE.Vector3(4.5, -2.75, -14.5),
        ];

        // Crear varillas moradas
        for (let i = 0; i < 3; i++) {
            const offset = new THREE.Vector3(-i * 1.1 - 3.45, 2, -3.85);
        ...
        const yellowRodPoints = [
            ...
            new THREE.Vector3(4.5, -2.75, -0.5),
            new THREE.Vector3(4.5, -2.75, -1),
            new THREE.Vector3(4.5, -2.75, -26.75),
        ];

        // Crear varillas amarillas
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 1.1 - 4, 2, 20.75);
        ...
        // Crear varillas amarillas
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(-i * 1.1 - 4, 2, -3.85);
        ...
        for (let i = 0; i < 5; i++) {
            const offset = new THREE.Vector3(0, -0.95, 18 - i * 2.5);
        ...
        const greenRodPoints2 = [
            new THREE.Vector3(0.6, 3.55, 0),
            new THREE.Vector3(1.25, 4.1, 0),
            new THREE.Vector3(1.25, 4.25, 0),
            ...
            new THREE.Vector3(-1.35, 4, 0),
            new THREE.Vector3(-1.35, 3, 0),
            ...
            new THREE.Vector3(1.25, 1, 0),
            new THREE.Vector3(1.25, 2, 0.05),
            new THREE.Vector3(1.25, 3, 0.1),
            new THREE.Vector3(1.25, 4, 0.15),
            new THREE.Vector3(1.25, 4.35, 0.25),

            new THREE.Vector3(1.15, 4.35, 0.25),
            new THREE.Vector3(1.05, 4.35, 0.25),
            new THREE.Vector3(1, 4.35, 0.25),
            new THREE.Vector3(0.4, 3.75, 0.25),
        ];

        // Crear varillas verdes
        for (let i = 0; i < 5; i++) {
            const offset = new THREE.Vector3(0, -0.95, 17 - i * 2.5);
        ...
        // Crear varillas verdes
        for (let i = 0; i < 5; i++) {
            const offset = new THREE.Vector3(0, -0.95, 1.5 - i * 2.2);
            const points = greenRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
        ...
        // Crear varillas verdes
        for (let i = 0; i < 5; i++) {
            const offset = new THREE.Vector3(0, -0.95, 0.5 - i * 2.2);
            const points = greenRodPoints2.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
        ...
        // Crear varillas verdes
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(0, -0.95, -14 - i * 2.2);
            const points = greenRodPoints.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
        ...
        // Crear varillas verdes
        for (let i = 0; i < 2; i++) {
            const offset = new THREE.Vector3(0, -0.95, -15 - i * 2.2);
            const points = greenRodPoints2.map(p => p.clone().add(offset));
            scene.current.add(createTube(points, 0x00ff00, 0.1));
        }
